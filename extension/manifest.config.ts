...
export default defineManifest({
  ...
  // web_accessible_resources are generated by crxjs from these content
  // scripts (each loader import()s its chunks via chrome.runtime.getURL), so
  // they get the same `matches` and `use_dynamic_url: false`. Deliberately
  // NOT switched to use_dynamic_url: true (fingerprinting hardening):
  //   - crxjs 2.x hardcodes false for manifest-declared content scripts;
  //   - more importantly it breaks loading. Verified in Chromium: getURL()
  //     does return the per-session dynamic URL and a top-level import() of
  //     it works, but that module's own relative imports (./disclose-version
  //     …) fail with "Failed to fetch dynamically imported module", and every
  //     overlay chunk has them.
  // `matches` can't narrow further either: the overlay runs on <all_urls>
  // and nav-reporter on http(s) frames, and their chunks must load there.
  content_scripts: [
    {
      matches: ["<all_urls>"],
